...
class WebVideoSystem:
    ...
    def capture_and_analyze(self, strategy="user-preference-first", session_id=None):
        ...
        try:
            ...
            ambiguity = True  # initialize empty; can be filled from past turns later
...
@app.route('/replay_last_reply', methods=['POST'])
def replay_last_reply():
    """Append a replay entry for the last robot reply and return it.

    This triggers external audio clients (like audio.py) that monitor
    conversation history to play the reply again.
    """
    try:
        msg = video_system.robot_service.append_robot_reply_replay()
        if not msg:
            return jsonify({"success": False, "error": "No robot reply to replay."}), 404
        # Keep status unchanged, but inform any listeners
        socketio.emit('status_update', {'status': video_system.current_status})
        return jsonify({
            "success": True,
            "robot_reply": msg.get('robot_reply', ''),
